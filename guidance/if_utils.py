...
class IF(nn.Module):
    def __init__(self, device, vram_O=False, t_range=[0.02, 0.98]):
        super().__init__()
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=7.5, grad_scale=1):
        ...
        images = (
            F.interpolate(pred_rgb, (64, 64), mode="bilinear", align_corners=False) * 2
            - 1
        )
    ...
    def train_step_perpneg(
        self, text_embeddings, weights, pred_rgb, guidance_scale=100, grad_scale=1
    ):
        B = pred_rgb.shape[0]
        ...
        with torch.no_grad():
            ...
            noise_pred = (
                noise_pred_uncond
                + guidance_scale
                * weighted_perpendicular_aggregator(delta_noise_preds, weights, B)
            )
        ...
        loss = 0.5 * F.mse_loss(images.float(), target, reduction='sum')
